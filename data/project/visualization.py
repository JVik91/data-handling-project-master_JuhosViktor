...
def songs_distribution(dataset: RentalDataset) -> None:
    labels = list(set({song.label for song in dataset.songs}))
    values = [0 for _ in labels]
    for song in dataset.songs:
        values[labels.index(song.label)] += 1
    ...
    ax.set_ylabel("Distribution of songs")
    ax.set_title("Songs for each company")
    ax.set_xticks(x)
